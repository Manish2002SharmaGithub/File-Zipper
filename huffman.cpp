...
void huffman::createArr() {
    for (int i = 0; i < 128; i++) {
        arr.push_back(new Node()); //creating new nodes
        arr[i]->data = i; 
        arr[i]->freq = 0;
    }
}
...
void huffman::createMinHeap() {
    ...
    //128 - ASCII encoding has 128 characters [0,127]
    for (int i = 0; i < 128; i++) {
        if (arr[i]->freq > 0) {
            minHeap.push(arr[i]);
        }
...
void huffman::createTree() {
    ...
    Node *left, *right;
    //real PQ data is copied so original is not lost
    priority_queue <Node*, vector<Node*>, Compare> tempPQ(minHeap);
    while (tempPQ.size() != 1){
        left = tempPQ.top();    tempPQ.pop();
        right = tempPQ.top();   tempPQ.pop();
